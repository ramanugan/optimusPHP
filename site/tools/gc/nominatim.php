<?
	...
	if(@$_GET['cmd'] == 'latlng')
	{
		...
		$lng = $_GET["lng"];
                ...
                $url = 'http://162.248.55.3/nominatim/reverse?email='.$gsValues['EMAIL'].'&format=json&lat='.$lat.'&lon='.$lng.'&zoom=18&addressdetails=1';
	...
	if(@$_GET['cmd'] == 'address')
	{
		...
		$search = htmlentities(urlencode($_GET["search"]));
                ...
                $url = 'https://nominatim.openstreetmap.org/search?email='.$gsValues['EMAIL'].'&q='.$search.'&format=json&polygon=0&addressdetails=1';
		$data = @file_get_contents($url);
		$jsondata = json_decode($data,true);
