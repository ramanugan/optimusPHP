...
function sendEmail($email, $subject, $message, $attachment = false)
{
	...
	if ($attachment !== false && $attachment !== null && $attachment !== '') {

		if (is_array($attachment)) {
			$tmp  = $attachment['tmp_name'] ?? null;
			$name = $attachment['name'] ?? null;

			if ($tmp && $name && file_exists($tmp)) {
...
function sendEmailReport($email, $subject, $message, $no_reply = false, $att_name = false, $att_str = false)
{
	require_once 'PHPMailerAutoload.php';
