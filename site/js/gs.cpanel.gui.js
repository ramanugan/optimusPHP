...
function initGui() {
  ...
  $("#manage_server_tabs, #dialog_object_edit_tabs_crm").tabs({});
...
function initGrids() {
  ...
  $("#cpanel_user_list_grid").jqGrid({
    ...
    datatype: "json",
    colNames: ['ID', la['USERNAME'], la['EMAIL'], la['ACTIVE'], la['EXPIRES_ON'], la['PRIVILEGES'], la['API'], la['REG_TIME'], la['LOGIN_TIME'], la['IP'], la['SUB_ACC'], la['OBJECTS'], la['EMAIL'], la['SMS'], la['WEBHOOK'], la['API'], '', ''],
    colModel: [
      ...
      { name: 'usage_webhook_daily_cnt', index: 'usage_webhook_daily_cnt', width: 50, align: "center" },
  ...
  $("#cpanel_user_list_grid").jqGrid('navGrid', '#cpanel_user_list_grid_pager', {
    add: true,
    edit: false,
    del: false,
    search: false,
  ...
  $("#cpanel_user_list_grid").navButtonAdd('#cpanel_user_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
  ...
  $("#cpanel_object_list_grid").jqGrid({
    ...
    datatype: "json",
    colNames: [la['NAME'], la['IMEI'], la['ACTIVE'], la['EXPIRES_ON'], la['SIM_CARD_NUMBER'],
    la['LAST_CONNECTION'], 'Dispositivos', 'Fecha Alta', 'Renta', 'Comentarios', la['STATUS'],
    la['USER_ACCOUNT'], '', ''],
    colModel: [
      ...
      { name: 'fecha_alta', index: 'fecha_alta', width: 85, sortable: false, align: "center" },
      { name: 'renta', index: 'renta', width: 60, sortable: false, align: "center" },
      { name: 'observacion', index: 'observacion', width: 100, sortable: false, align: "left" },
  ...
  $("#cpanel_object_list_grid").jqGrid('navGrid', '#cpanel_object_list_grid_pager', {
    add: true,
    edit: false,
    del: false,
    search: false,
  ...
  $("#cpanel_object_list_grid").navButtonAdd('#cpanel_object_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
    buttonicon: 'ui-icon-action',
    onClickButton: function () { },
  ...
  // define imei-device list grid  **********	
  $("#cpanel_imei_device_list_grid").jqGrid({
    url: 'func/fn_cpanel.objects.php?cmd=load_imei_device_list',
    datatype: "json",
    colNames: [la['IMEI'], la['BRANDING'], la['MODEL'], la['EXPENSE'],
    la['COST'], la['PURCHASE_DATE'], la['ALTA_DATE'], '', ''],
    colModel: [
      { name: 'imei', index: 'imei', width: 80 },
      { name: 'marca', index: 'marca', width: 50, align: "center" },
      { name: 'modelo', index: 'modelo', width: 60, align: "center" },
      { name: 'proveedor', index: 'proveedor', width: 80 },
      { name: 'costo', index: 'costo', width: 80, align: "center" },
      { name: 'fecha_compra', index: 'fecha_compra', width: 60, align: "center" },
      { name: 'fecha_alta', index: 'fecha_alta', width: 85, sortable: false, align: "center" },
      { name: 'modify', index: 'modify', width: 75, align: "center", sortable: false, fixed: true },
      { name: 'scroll_fix', index: 'scroll_fix', width: 13, sortable: false, fixed: true } // scroll fix
    ],
    rowNum: 50,
    rowList: [25, 50, 100, 200, 300, 400, 500],
    pager: '#cpanel_imei_device_list_grid_pager',
    sortname: 'imei',
    sortorder: "asc",
    viewrecords: true,
    rownumbers: true,
  ...
  $("#cpanel_imei_device_list_grid").jqGrid('navGrid', '#cpanel_imei_device_list_grid_pager', {
    add: true,
    edit: false,
    del: false,
    search: false,
  ...
  $("#cpanel_imei_device_list_grid").navButtonAdd('#cpanel_imei_device_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
    buttonicon: 'ui-icon-action',
    onClickButton: function () { },
    position: "last",
    id: "cpanel_imei_device_list_grid_action_menu_button"
  });

  // action menu
  $("#cpanel_imei_device_list_grid_action_menu").menu({
    role: 'listbox'
  });
  $("#cpanel_imei_device_list_grid_action_menu").hide();

  $("#cpanel_imei_device_list_grid_action_menu_button").click(function () {
    $("#cpanel_imei_device_list_grid_action_menu").toggle().position({
      my: "left bottom",
      at: "right-5 top-5",
      of: this
    });
  ...
  $("#cpanel_imei_device_list_grid").setCaption('<div class="row4">\
								<input id="cpanel_imei_device_list_search" class="inputbox-search" type="text" value="" placeholder="'+ la['SEARCH'] + '" maxlength="25">\
							</div>');

  $("#cpanel_imei_device_list_search").bind("keyup", function (e) {
    var manager_id = '&manager_id=' + cpValues['manager_id'];
    $('#cpanel_imei_device_list_grid').setGridParam({ url: 'func/fn_cpanel.objects.php?cmd=load_object_list&s=' + this.value + manager_id });
    $('#cpanel_imei_device_list_grid').trigger("reloadGrid");
  });

  $("#cpanel_imei_device_list_grid").setGridWidth($(window).width() - 60);
  $("#cpanel_imei_device_list_grid").setGridHeight($(window).height() - 207);
  $(window).bind('resize', function () { $("#cpanel_imei_device_list_grid").setGridWidth($(window).width() - 60); });
  $(window).bind('resize', function () { $("#cpanel_imei_device_list_grid").setGridHeight($(window).height() - 207); });

  // define imei list grid *************
  $("#cpanel_imei_list_grid").jqGrid({
    url: 'func/fn_cpanel.objects.php?cmd=load_imei_list',
    datatype: "json",
    colNames: [la['IMEI'], la['PHONE_NUMBER'], la['SIM_CARD_ICCID'], la['PLAN'], la['RENT_COST'], la['SIM_CARD_COMPANY'], la['EXPENSE'], la['DATE_CLOSE'], la['PURCHASE_DATE'], la['ALTA_DATE'], '', ''],
    colModel: [
      { name: 'IMEI', index: 'imei', width: 80 },
      { name: 'numero_linea', index: 'numero_linea', width: 80 },
      { name: 'iccid', index: 'iccid', width: 80 },
      { name: 'plan', index: 'plan', width: 50, align: "center" },
      { name: 'renta_costo', index: 'renta_costo', width: 60, align: "center" },
      { name: 'sim_number_company', index: 'sim_number_company', width: 80 },
      { name: 'proveedor', index: 'proveedor', width: 60, align: "center" },
      { name: 'fecha_corte', index: 'fecha_corte', width: 80, align: "center" },
      { name: 'fecha_compra', index: 'fecha_compra', width: 80, align: "center" },
      { name: 'fecha_alta', index: 'fecha_alta', width: 85, sortable: false, align: "center" },
      { name: 'modify', index: 'modify', width: 75, align: "center", sortable: false, fixed: true },
      { name: 'scroll_fix', index: 'scroll_fix', width: 13, sortable: false, fixed: true } // scroll fix
    ],
    rowNum: 50,
    rowList: [25, 50, 100, 200, 300, 400, 500],
    pager: '#cpanel_imei_list_grid_pager',
    sortname: 'imei',
    sortorder: "asc",
    viewrecords: true,
    rownumbers: true,
  ...
  $("#cpanel_imei_list_grid").jqGrid('navGrid', '#cpanel_imei_list_grid_pager', {
    add: true,
    edit: false,
    del: false,
    search: false,
  ...
  $("#cpanel_imei_list_grid").navButtonAdd('#cpanel_imei_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
    buttonicon: 'ui-icon-action',
    onClickButton: function () { },
    position: "last",
    id: "cpanel_imei_list_grid_action_menu_button"
  });

  // action menu
  $("#cpanel_imei_list_grid_action_menu").menu({
    role: 'listbox'
  });
  $("#cpanel_imei_list_grid_action_menu").hide();

  $("#cpanel_imei_list_grid_action_menu_button").click(function () {
    $("#cpanel_imei_list_grid_action_menu").toggle().position({
      my: "left bottom",
      at: "right-5 top-5",
      of: this
    });
  ...
  $("#cpanel_imei_list_grid").setCaption('<div class="row4">\
								<input id="cpanel_imei_list_search" class="inputbox-search" type="text" value="" placeholder="'+ la['SEARCH'] + '" maxlength="25">\
							</div>');

  $("#cpanel_imei_list_search").bind("keyup", function (e) {
    var manager_id = '&manager_id=' + cpValues['manager_id'];
    $('#cpanel_imei_list_grid').setGridParam({ url: 'func/fn_cpanel.objects.php?cmd=load_object_list&s=' + this.value + manager_id });
    $('#cpanel_imei_list_grid').trigger("reloadGrid");
  });

  $("#cpanel_imei_list_grid").setGridWidth($(window).width() - 60);
  $("#cpanel_imei_list_grid").setGridHeight($(window).height() - 207);
  $(window).bind('resize', function () { $("#cpanel_imei_list_grid").setGridWidth($(window).width() - 60); });
  $(window).bind('resize', function () { $("#cpanel_imei_list_grid").setGridHeight($(window).height() - 207); });
  ...
  if (document.getElementById('cpanel_unused_object_list_grid') != undefined) {
    $("#cpanel_unused_object_list_grid").jqGrid({
      ...
      datatype: "json",
      colNames: [la['USER'], la['NAME'], la['IMEI'], la['SIM_CARD_NUMBER'], la['LAST_CONNECTION'], la['DT_EDIT'], la['REPORT_ATTEMPTS'], la['COMMENT'], la['STATUS'], la['FOLLOW'], '', ''],
      colModel: [
        { name: 'user', index: 'user', width: 100 },
        { name: 'name', index: 'name', width: 80 },
        { name: 'imei', index: 'imei', width: 80, align: "center" },
        { name: 'sim_number', index: 'sim_number', width: 100, align: "center" },
        { name: 'dt_server', index: 'dt_server', width: 80, align: "center" },
        { name: 'dt_edit', index: 'dt_edit', width: 80, align: "center" },
        { name: 'numero_report', index: 'numero_report', width: 100, align: "center" },
        { name: 'observacion', index: 'observacion', width: 100, sortable: false },
        { name: 'status', index: 'status', width: 35, sortable: false, align: "center" },
        { name: 'modify', index: 'modify', width: 75, align: "center", sortable: false, fixed: true },
        { name: 'follow', index: 'follow', width: 35, align: "center", sortable: false, fixed: true },
        { name: 'scroll_fix', index: 'scroll_fix', width: 13, sortable: false, fixed: true } // scroll fix
      ],
      rowNum: 50,
      rowList: [20, 50, 100, 200, 300, 400, 500],
    ...
    $("#cpanel_unused_object_list_grid").jqGrid('navGrid', '#cpanel_unused_object_list_grid_pager', {
      add: false,
      edit: false,
      del: false,
      search: false
    ...
    $("#cpanel_unused_object_list_grid").navButtonAdd('#cpanel_unused_object_list_grid_pager', {
      caption: "",
      title: la['ACTION'],
      buttonicon: 'ui-icon-action',
      onClickButton: function () { },
  ...
  if (document.getElementById("cpanel_billing_plan_list_grid") != undefined) {
    ...
    $("#cpanel_billing_plan_list_grid").jqGrid('navGrid', '#cpanel_billing_plan_list_grid_pager', {
      add: false,
      edit: false,
      del: false,
      search: false
    ...
    $("#cpanel_billing_plan_list_grid").navButtonAdd('#cpanel_billing_plan_list_grid_pager', {
      caption: "",
      title: la['ACTION'],
      buttonicon: 'ui-icon-action',
      onClickButton: function () { },
  ...
  $("#dialog_user_edit_subaccount_list_grid").setCaption('<div class="row4"><input id="dialog_user_edit_subaccount_list_search" class="inputbox-search" type="text" value="" placeholder="' + la['SEARCH'] + '" maxlength="25"></div>');

  $("#dialog_user_edit_subaccount_list_search").bind("keyup", function (e) {
    $('#dialog_user_edit_subaccount_list_grid').setGridParam({ url: 'func/fn_cpanel.users.php?cmd=load_user_subaccount_list&id=' + cpValues['user_edit_id'] + '&s=' + this.value });
    $('#dialog_user_edit_subaccount_list_grid').trigger("reloadGrid");
  });

  $("#dialog_user_edit_subaccount_list_grid").jqGrid('navGrid', '#dialog_user_edit_subaccount_list_grid_pager', {
    add: false,
    edit: false,
    del: false,
    search: false
  ...
  $("#dialog_user_edit_subaccount_list_grid").navButtonAdd('#dialog_user_edit_subaccount_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
    buttonicon: 'ui-icon-action',
    onClickButton: function () { },
  ...
  $("#dialog_user_edit_object_list_grid").setCaption('<div class="row4"><input id="dialog_user_edit_object_list_search" class="inputbox-search" type="text" value="" placeholder="' + la['SEARCH'] + '" maxlength="25"></div>');

  $("#dialog_user_edit_object_list_search").bind("keyup", function (e) {
    $('#dialog_user_edit_object_list_grid').setGridParam({ url: 'func/fn_cpanel.users.php?cmd=load_user_object_list&id=' + cpValues['user_edit_id'] + '&s=' + this.value });
    $('#dialog_user_edit_object_list_grid').trigger("reloadGrid");
  });

  $("#dialog_user_edit_object_list_grid").jqGrid('navGrid', '#dialog_user_edit_object_list_grid_pager', {
    add: true,
    edit: false,
    del: false,
    search: false,
  ...
  $("#dialog_user_edit_object_list_grid").navButtonAdd('#dialog_user_edit_object_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
    buttonicon: 'ui-icon-action',
    onClickButton: function () { },
  ...
  if (document.getElementById("dialog_user_edit_billing_plan_list_grid") != undefined) {
    ...
    $("#dialog_user_edit_billing_plan_list_grid").jqGrid('navGrid', '#dialog_user_edit_billing_plan_list_grid_pager', {
      add: true,
      edit: false,
      del: false,
      search: false,
    ...
    $("#dialog_user_edit_billing_plan_list_grid").navButtonAdd('#dialog_user_edit_billing_plan_list_grid_pager', {
      caption: "",
      title: la['ACTION'],
      buttonicon: 'ui-icon-action',
      onClickButton: function () { },
  ...
  $("#dialog_user_edit_usage_list_grid").jqGrid({
    url: 'func/fn_cpanel.users.php',
    datatype: "json",
    colNames: [la['DATE'], la['LOGIN'], la['EMAIL'], la['SMS'], la['WEBHOOK'], la['API'], ''],
    colModel: [
      { name: 'dt_usage', index: 'dt_usage', width: 135, align: "center" },
      { name: 'login', index: 'login', width: 104, align: "center" },
      { name: 'email', index: 'email', width: 104, align: "center" },
      { name: 'sms', index: 'sms', width: 104, align: "center" },
      { name: 'webhook', index: 'webhook', width: 104, align: "center" },
      { name: 'api', index: 'api', width: 104, align: "center" },
      { name: 'modify', index: 'modify', width: 60, align: "center", sortable: false },
    ],
    rowNum: 25,
    rowList: [25, 50, 100, 200, 300, 400, 500],
  ...
  $("#dialog_user_edit_usage_list_grid").jqGrid('navGrid', '#dialog_user_edit_usage_list_grid_pager', {
    add: false,
    edit: false,
    del: false,
    search: false,
  ...
  $("#dialog_user_edit_usage_list_grid").navButtonAdd('#dialog_user_edit_usage_list_grid_pager', {
    caption: "",
    title: la['ACTION'],
    buttonicon: 'ui-icon-action',
    onClickButton: function () { },
...
function initSelectList(list) {
  switch (list) {
    case "object_device_list":
      ...
      for (var key in gsValues['protocol_list']) {
        var obj = gsValues['protocol_list'][key];
...
function switchCPTab(name) {
  ...
  // document.getElementById("top_panel_sim").className = "imei-server-btn";
  // document.getElementById("top_panel_imei_device").className = "device-server-btn";
  ...
  switch (name) {
    case "user_list":
      ...
      document.getElementById('cpanel_imei_list').style.display = 'none';
      document.getElementById('cpanel_imei_device_list').style.display = 'none';
    ...
    case "imei_list":
      $('#cpanel_imei_list_grid').trigger("reloadGrid");
      document.getElementById("top_panel_sim").className = "imei-server-btn active";

      document.getElementById('cpanel_imei_list').style.display = '';
      document.getElementById('cpanel_user_list').style.display = 'none';
      document.getElementById('cpanel_object_list').style.display = 'none';
      document.getElementById('cpanel_imei_device_list').style.display = 'none';
      ...
      break;
    case "imei_device_list":
      $('#cpanel_imei_device_list_grid').trigger("reloadGrid");
      document.getElementById("top_panel_imei_device").className = "device-server-btn active";
      
      document.getElementById('cpanel_imei_device_list').style.display = '';
      document.getElementById('cpanel_user_list').style.display = 'none';
      document.getElementById('cpanel_object_list').style.display = 'none';
      document.getElementById('cpanel_imei_list').style.display = 'none';
