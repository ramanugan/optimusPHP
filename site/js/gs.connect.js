...
function notifyDialogObjects(text) {
	document.getElementById('dialog_notify_text_Objects').innerHTML = text;

	$('#dialog_notify_Objects').dialog('open');
}
...
function connectLogin()
{
	...
	jQuery.ajax({
		...
		success: function(result)
		{
			...
			else if (result == 'SOME_OBJECTS_EXCEEDED')
			{
				notifyDialogObjects(la["OBJECTS_EXCEEDED_REPORTS"]);
			}
...
function connectRecover(token)
{
	...
	jQuery.ajax({
		...
		success: function(result)
		{
		    	if (result == 'OK')
			{
                                notifyDialog(la['USERNAME_PASSWORD_SENT'] + ' ' + la['PLEASE_CHECK_SERVER_EMAIL_SMTP_SETTINGS']);
                        }
			else if (result == 'ERROR_NOT_SENT')
			{
