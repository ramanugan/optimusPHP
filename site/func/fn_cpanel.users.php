...
if (@$_GET['cmd'] == 'load_user_list') {
	...
	$q = "SELECT gs_users.*,
			...
			(SELECT COUNT(DISTINCT (`imei`)) FROM gs_user_objects WHERE gs_user_objects.user_id = gs_users.id) as obj_cnt
			FROM gs_users";
	...
	if ($r) {
		...
		while ($row = mysqli_fetch_array($r)) {
			...
			$privileges = '';
			if ($row['privileges']['type'] == 'super_admin') {
				$privileges = $la['SUPER_ADMINISTRATOR'];
			}
			if ($row['privileges']['type'] == 'admin') {
				$privileges = $la['ADMINISTRATOR'];
			}
			if ($row['privileges']['type'] == 'manager') {
				$privileges = $la['MANAGER'];
			}
			if ($row['privileges']['type'] == 'user') {
				$privileges = $la['USER'];
			}
			//if ($row['privileges']['type'] == 'user') {$row['privileges']['show_update_dialog'] = true;}
			if ($row['privileges']['type'] == 'viewer') {
				$privileges = $la['VIEWER'];
			}
			...
			//$privileges_ = json_encode($row['privileges']);
			//$id = $row['id'];

			//$q0 = "UPDATE `gs_users` SET `privileges` ='" . $privileges_ . "' WHERE `id`='" . $id . "'";
			...
			// check if any object expire plan soon
			while ($row2 = mysqli_fetch_array($r2)) {
				$imei = $row2['imei'];
				$q3 = "SELECT * FROM `gs_object_services` WHERE imei='" . $imei . "'";
				$r3 = mysqli_query($ms, $q3);

				if ($r3) {
					$row3 = mysqli_fetch_assoc($r3);
					if ($row3) {
						$dia_limite = $row3['days_left_num'];
						$plan = $row3['plan'];

						if ($plan == 'Garantia' || $plan == 'Mantenimiento') {
							$days_diff = strtotime(gmdate("Y-m-d")) - (strtotime($row3['days_last']));
...
if (@$_POST['cmd'] == 'register_user') {
	...
	if ($email != '') {
		...
		$privileges['show_update_dialog'] = stringToBool($gsValues['USER_SHOW_UPDATE_DIALOG']);
		...
		$ALL_PERM_KEYS = array(
		  ...
		  'perm_crear_reportes',
		  'perm_ver_reportes',
		  'perm_eliminar_reportes',
		  'perm_editar_reportes',
		
		  // 🔹 Mantenimiento
		  'perm_crear_mantenimiento',
		  'perm_eliminar_mantenimientos',
		  'perm_editar_mantenimientos',
		
		  // 🔹 Comandos
		  'perm_crear_comandos_gprs',
		  'perm_crear_comandos_sms',
		  'perm_plantillas_comandos',
		  'perm_programar_comandos',
		
		  // 🔹 Mapa
		  'perm_activar_desactivar_tools',
		  'perm_marcadores_mapa',
		  'perm_rutas_mapa',
		  'perm_geocercas_mapa',
		);
		
		foreach ($ALL_PERM_KEYS as $k) {
		  $privileges[$k] = false;
		}
		...
		if (isset($PLAN_TRUE[$plan])) {
		  foreach ($PLAN_TRUE[$plan] as $k) {
			$privileges[$k] = true;
		  }
		}
		
		$privileges['plan'] = $plan;
		...
		$result = addUser($send, 'true', $info, '', '', $privileges, $manager_id, $email, $email, '', $gsValues['OBJ_ADD'], $gsValues['OBJ_LIMIT'], $gsValues['OBJ_LIMIT_NUM'], $gsValues['OBJ_DAYS'], $gsValues['OBJ_DAYS_NUM'], $gsValues['OBJ_EDIT'], $gsValues['OBJ_DELETE'], $gsValues['OBJ_HISTORY_CLEAR'],'','');

		if ($result == 'OK') { // attachements docs to new user
			$last_id = mysqli_insert_id($ms);

			$MiContrato = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['MiContrato']);
			$ActaConstitutiva = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['ActaConstitutiva']);
			$RepLegal = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['RepLegal']);
			$IdentificacionOficial = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['IdentificacionOficial']);
			$OpinionPositiva = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['OpinionPositiva']);
			$ConstanciaFiscal = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['ConstanciaFiscal']);
			$DomicilioFiscal = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['DomicilioFiscal']);
			$Otros = preg_replace('/\_\d+/', '_user_id_' . $last_id, $_POST['Otros']);

			$q1 = "INSERT INTO gs_user_docs (user_id, doc_1, doc_2, doc_3, doc_4, doc_5, doc_6, doc_7, doc_8) VALUES (" . $last_id . ", '" . $MiContrato . "', '" . $ActaConstitutiva . "', '" . $RepLegal . "', '" . $IdentificacionOficial . "', '" . $OpinionPositiva . "', '" . $ConstanciaFiscal . "', '" . $DomicilioFiscal . "', '" . $Otros . "')";
			mysqli_query($ms, $q1);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['MiContrato'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $MiContrato);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['ActaConstitutiva'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $ActaConstitutiva);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['RepLegal'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $RepLegal);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['IdentificacionOficial'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $IdentificacionOficial);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['OpinionPositiva'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $OpinionPositiva);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['ConstanciaFiscal'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $ConstanciaFiscal);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['DomicilioFiscal'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $DomicilioFiscal);
			rename($gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $_POST['Otros'], $gsValues['PATH_ROOT'] . 'data/user/settingsUsersDocument/' . $Otros);
		}
		addRowBinnacle($_SESSION["user_id"], 'Alta de usuario: ' . $email, $q);
		echo $result;
	}
	die;
}
...
if (@$_POST['cmd'] == 'login_user') {
	...
	addRowBinnacle($_SESSION["user_id"], 'Entro como usuario a: ' . $username, $q);
...
if (@$_POST['cmd'] == 'load_user_data') {
	...
	$q1 = "SELECT doc_1,doc_2,doc_3,doc_4,doc_5,doc_6,doc_7,doc_8 FROM `gs_user_docs` WHERE `user_id`='" . $id . "'";
	$r1 = mysqli_query($ms, $q1);
	$docs = mysqli_fetch_assoc($r1);
	$q1 = "SELECT doc_1,doc_2,doc_3,doc_4,doc_5,doc_6,doc_7,doc_8 FROM `gs_user_docs` WHERE `user_id`='" . $id . "'";
	$r1 = mysqli_query($ms, $q1);
	$docs = mysqli_fetch_assoc($r1);
	...
	$result = array(
		...
		'usage_webhook_daily' => $row['usage_webhook_daily'],
...
if (@$_POST['cmd'] == 'edit_user') {
	...
	$usage_webhook_daily = $_POST['usage_webhook_daily'];
	...
	$number = isset($infoArray['number']) ? $infoArray['number'] : null;


	// $user = $_SESSION['user_id'];
	// if (!checkCPanelToObjectUserPrivilegesIncuded($user)) {
	// 	echo 'FALSE';
	// 	die;
	// }
	$q = "UPDATE `gs_user_objects` SET `client_id`='" . $number . "' WHERE `user_id`='" . $id . "'";
	$r = mysqli_query($ms, $q);
	...
	if (is_string($privileges)) {
		$privileges_arr = json_decode($privileges, true);
	} else {
		$privileges_arr = $privileges;
	}
	if (!is_array($privileges_arr)) {
		$privileges_arr = [];
	}
	...
	if (count($perm_to_apply) > 0) {
		$q = "SELECT id, privileges FROM gs_users WHERE manager_id = $parent_id";
		$r = mysqli_query($ms, $q);

		if ($r) {
			while ($sub = mysqli_fetch_assoc($r)) {
				$sub_id = (int)$sub['id'];

				$sub_priv = json_decode((string)$sub['privileges'], true);
				if (!is_array($sub_priv)) {
					$sub_priv = [];
				}

				foreach ($perm_to_apply as $pk => $pv) {
					$sub_priv[$pk] = $pv;
				}

				$new_json = json_encode($sub_priv, JSON_UNESCAPED_UNICODE);
				$new_json_esc = mysqli_real_escape_string($ms, $new_json);

				$uq = "UPDATE gs_users SET privileges = '$new_json_esc' WHERE id = $sub_id";
				mysqli_query($ms, $uq);
			}
		}
	}

	$privileges_json = json_encode($privileges_arr, JSON_UNESCAPED_UNICODE);
	$privileges_json_esc = mysqli_real_escape_string($ms, $privileges_json);

	$q = "UPDATE `gs_users` SET
        `privileges`='$privileges_json_esc',
        `manager_id`='" . mysqli_real_escape_string($ms, $manager_id) . "',
        `manager_billing`='" . mysqli_real_escape_string($ms, $manager_billing) . "',
        `username`='" . mysqli_real_escape_string($ms, $username) . "',
        `email`='" . mysqli_real_escape_string($ms, $email) . "',
        `api`='" . mysqli_real_escape_string($ms, $api) . "',
        `api_key`='" . mysqli_real_escape_string($ms, $api_key) . "',
        `info`='" . mysqli_real_escape_string($ms, $info) . "',
        `obj_add`='" . mysqli_real_escape_string($ms, $obj_add) . "',
        `obj_limit`='" . mysqli_real_escape_string($ms, $obj_limit) . "',
        `obj_limit_num`='" . mysqli_real_escape_string($ms, $obj_limit_num) . "',
        `obj_days`='" . mysqli_real_escape_string($ms, $obj_days) . "',
        `obj_days_dt`='" . mysqli_real_escape_string($ms, $obj_days_dt) . "',
        `obj_edit`='" . mysqli_real_escape_string($ms, $obj_edit) . "',
        `obj_delete`='" . mysqli_real_escape_string($ms, $obj_delete) . "',
        `obj_history_clear`='" . mysqli_real_escape_string($ms, $obj_history_clear) . "',
        `sms_gateway_server`='" . mysqli_real_escape_string($ms, $sms_gateway_server) . "',
        `places_markers`='" . mysqli_real_escape_string($ms, $places_markers) . "',
        `places_routes`='" . mysqli_real_escape_string($ms, $places_routes) . "',
        `places_zones`='" . mysqli_real_escape_string($ms, $places_zones) . "',
        `usage_email_daily`='" . mysqli_real_escape_string($ms, $usage_email_daily) . "',
        `usage_sms_daily`='" . mysqli_real_escape_string($ms, $usage_sms_daily) . "',
        `usage_webhook_daily`='" . mysqli_real_escape_string($ms, $usage_webhook_daily) . "',
        `usage_api_daily`='" . mysqli_real_escape_string($ms, $usage_api_daily) . "'
      WHERE `id`='$parent_id'";
	...
	if ($password != '') {
		...
		$q = "SELECT * FROM `gs_users` WHERE `id`='" . $pid . "' LIMIT 1";
		$r = mysqli_query($ms, $q);
		$row = mysqli_fetch_array($r);
		

		$info = json_decode($row['info'], true);
		$email_user = $info['email2'] ?? '';

		$emails = [];

		if (!empty($email_admin) && $email_admin !== 'undefined') {
			$emails[] = $email_admin;
		}

		if (!empty($email_user) && $email_user !== 'undefined') {
			$emails[] = $email_user;
		}
		...
		if (!empty($email_list)) {
			...
			$message = str_replace(
				"*No olvide cambiar la contraseña al ingresar",
				"*Por motivos de seguridad, le recomendamos cambiar su contraseña inmediatamente después de iniciar sesión y eliminar este mensaje.

Este mensaje fue generado automáticamente por el sistema. Por favor, no responda.",
				$message
			);

			if (sendEmail($email_list, $subject, $message)) {
				$q = "UPDATE gs_users SET password='" . md5($new_password) . "' WHERE email='" . $email . "' AND `id`='$pid'";
				$r = mysqli_query($ms, $q);

			} else {
				echo 'ERROR_NOT_SENT';
				addRowBinnacle($_SESSION["user_id"], 'Edición password Fallido:' . $username, $q);
			}
		} else {
	...
	addRowBinnacle($_SESSION["user_id"], 'Edición password:' . $username, $q);
	echo 'OK';
	die;
}
...
if (@$_POST['cmd'] == 'activate_user') {
	...
	if ($_SESSION["user_id"] != $id) {
		...
		$row = mysqli_fetch_array($r);
		$username = $row['username'];
		addRowBinnacle($_SESSION["user_id"], 'Activacion de usuario: ' . $username, $q);

		$q = "UPDATE `gs_users` SET `active`='true' WHERE `id`='" . $id . "' OR (`manager_id`='" . $id . "' AND `manager_id` IS NOT NULL)";
		$r = mysqli_query($ms, $q);
	}
...
if (@$_POST['cmd'] == 'deactivate_user') {
	...
	if ($_SESSION["user_id"] != $id) {
		...
		$row = mysqli_fetch_array($r);
		$username = $row['username'];
		addRowBinnacle($_SESSION["user_id"], 'Desactivacion de usuario: ' . $username, $q);

		$q = "UPDATE `gs_users` SET `active`='false' WHERE `id`='" . $id . "' OR (`manager_id`='" . $id . "' AND `manager_id` IS NOT NULL)";
		$r = mysqli_query($ms, $q);
	}
...
if (@$_POST['cmd'] == 'delete_selected_users') // ya esta en bitacora con la funcion "delUser"
{
	$user = $_SESSION['user_id'];
	if (!checkCPanelToObjectUserPrivileges($user)) {
		echo 'FALSE';
...
if (@$_GET['cmd'] == 'load_user_object_list') {
	...
	// check if admin or manager
	$q = "SELECT DISTINCT (gs_objects.imei), gs_objects.*
			FROM gs_objects
	...
	if ($search != '') {
		$q .= " AND (UPPER(gs_objects.imei) LIKE '%$search%' OR UPPER(gs_objects.name) LIKE '%$search%')";
	}
...
if (@$_POST['cmd'] == 'delete_user_object') {
	...
	addRowBinnacle($_SESSION["user_id"], 'Se elimina equipo de usuario:' . $username . " gps: " . $imei, $q);
	echo 'OK';
	die;
}
...
if (@$_POST['cmd'] == 'activate_selected_user_objects') {
	...
	for ($i = 0; $i < count($imeis); ++$i) {
		...
		$q1 = "SELECT * FROM `gs_users` WHERE `id`='" . $user_id . "'";
		$r1 = mysqli_query($ms, $q1);
		$row1 = mysqli_fetch_array($r1);
		$username = $row1['username'];
		addRowBinnacle($_SESSION["user_id"], 'Se activa equipo de usuario:' . $username . " gps: " . $imei, $q);
	}
...
if (@$_POST['cmd'] == 'deactivate_selected_user_objects') {
	...
	for ($i = 0; $i < count($imeis); ++$i) {
		...
		$row1 = mysqli_fetch_array($r1);
		$username = $row1['username'];
		addRowBinnacle($_SESSION["user_id"], 'Se desactiva equipo de usuario:' . $username . " gps: " . $imei, $q);
	}
...
if (@$_GET['cmd'] == 'load_user_usage_list') {
	...
	if ($r) {
		...
		while ($row = mysqli_fetch_array($r)) {
			...
			$response->rows[$i]['cell'] = array($date, $login, $email, $sms, $webhook, $api, $modify);
			$i++;
		}
	}
