	...
	{		
		...
		else
		{
			...
			if ($_SESSION["privileges"] == 'subuser')
			{
				...
				$q = "SELECT * FROM `gs_user_objects`
				WHERE `user_id`='".$user_id ."' AND `imei` IN (". implode(',', $sub_imeis) .") ORDER BY `imei`, `group_id` ASC";
			}
			else
			{
				$q = "SELECT * FROM `gs_user_objects` WHERE `user_id`='".$user_id ."' ORDER BY `imei`, `group_id` ASC";
			}	
		}
		...
		while($row = mysqli_fetch_array($r))
		{
			...
			if ($active == 'true'){
				...
				$result[$imei]['dt_last_idle'] = $row2['dt_last_idle'];
				...
				// stop_engine
				$bloqueo = $params['out1'] ?? $params['output'] ?? null;
				if ($bloqueo){
                  $result[$imei]['st'] = 'B';
				}
				...
				if ( preg_match('/(\d+)_\d+/', $imei, $res )) {
					$imei_orig= $res[1];
					$q3 = "SELECT * FROM `gs_object_services` WHERE `imei`='". $imei_orig ."' ORDER BY name asc";
				} else {
				...
				while($row3 = mysqli_fetch_array($r3)) {
					...
					if (count($left_arr) > 0)
					{
						$status = $la["LEFT_MTTO"].' ('.implode(", ", $left_arr).')';
					}
