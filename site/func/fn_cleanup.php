<?
		...
		if (count($user_ids) > 0)
		{
			...
			while ($row = mysqli_fetch_array($r))
			{
				...
				if (!in_array($user_id, $user_ids))
				{
					$q2 = "DELETE FROM `gs_user_kml` WHERE `user_id`='".$user_id."'";				
					$r2 = mysqli_query($ms, $q2);
			...
			while ($row = mysqli_fetch_array($r))
			{
				...
				if (!in_array($user_id, $user_ids))
				{
					...
					$count++;
                        ...
                        // gs_user_share_position
			$q = "SELECT * FROM `gs_user_share_position`";
			$r = mysqli_query($ms, $q);
			...
			while ($row = mysqli_fetch_array($r))
			{
				...
				if (!in_array($user_id, $user_ids))
				{
					$q2 = "DELETE FROM `gs_user_share_position` WHERE `user_id`='".$user_id."'";				
					$r2 = mysqli_query($ms, $q2);
