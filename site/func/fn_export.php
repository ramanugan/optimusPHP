<?
	...
	if (@$_GET['format'] == 'events_csv')
	{
		// check privileges
		if ($_SESSION["privileges"] == 'subuser')
		{
			$q = "SELECT * FROM `gs_user_last_events_data` WHERE `user_id`='".$user_id."' AND `imei` IN (".$_SESSION["privileges_imei"].") ORDER BY `dt_tracker` DESC";
		}
		else
		{
			$q = "SELECT * FROM `gs_user_last_events_data` WHERE `user_id`='".$user_id."' ORDER BY `dt_tracker` DESC";			
		}		
		$r = mysqli_query($ms, $q);
		...
		echo 'dt,imei,object,model,vin,plate_number,device,sim_number,event,lat,lng,altitude,angle,speed,params';
		echo "\r\n";
		...
		while($row=mysqli_fetch_array($r))
		{
			...
			echo $dt.','.$imei.','.$name.','.$model.','.$vin.','.$plate_number.','.$device.','.$sim_number.','.$event.','.$lat.','.$lng.','.$altitude.','.$angle.','.$speed.',"'.$params_.'"';
			echo "\r\n";
		}	
	}
	if (@$_GET['format'] == 'eventsManagment_csv')
	{
		// check privileges
		if ($_SESSION["privileges"] == 'subuser')
		{
		...
		if (isset( $_GET['search'] ) && strlen($_GET['search']) > 0) {
			$q.= "AND name LIKE '%" . $_GET['search'] . "%'
				  OR event_desc LIKE '%" . $_GET['search'] . "%'
				  AND dt_tracker > date_sub(now(), interval 3 MONTH)";
		}
		...
		echo 'dt,imei,object,model,vin,plate_number,device,sim_number,event,lat,lng,altitude,angle,speed,tracing';
		echo "\r\n";
		...
		while($row=mysqli_fetch_array($r))
		{
			...
			$q3 = "SELECT details, fecha, gs_users.username  FROM gs_user_last_events_data_details
				   INNER JOIN gs_users ON gs_users.id = gs_user_last_events_data_details.user_id 
				   WHERE event_id=". $row['event_id'];
			$r3 = mysqli_query($ms, $q3);
			$seguimiento = '';
			while ($row_detail = mysqli_fetch_assoc($r3)) {
				$seguimiento .= '||';
				$seguimiento .= $row_detail['username'];
				$seguimiento .= '|';
				$seguimiento .= $row_detail['fecha'];
				$seguimiento .= '|';
				$seguimiento .= $row_detail['details'];
			}
			if ($r3->num_rows > 0)
				$seguimiento.= '||';
						
			echo $dt.','.$imei.','.$name.','.$model.','.$vin.','.$plate_number.','.$device.','.$sim_number.','.$event.','.$lat.','.$lng.','.$altitude.','.$angle.','.$speed.',"'.$seguimiento.'"';
			echo "\r\n";
		}	
	}
	...
	if (@$_GET['format'] == 'history_csv')
	{
		...
		echo 'dt,lat,lng,altitude,angle,speed,address,params';
		echo "\r\n";
		
		for ($i=0; $i<count($route); ++$i)
		{
			$address = getGeocoderCache($route[$i][1], $route[$i][2]);
			$dt = $route[$i][0];
			$lat = $route[$i][1];
			$lng = $route[$i][2];
			$altitude = $route[$i][3];
			...
			$address_ = str_replace(',', '', $address);
			
			echo $dt.','.$lat.','.$lng.','.$altitude.','.$angle.','.$speed.','.$address_.',"'.$params_.'"';
			echo "\r\n";
		}
	}
	...
	if (@$_GET['format'] == 'binnacle_csv') {
		
		header('Content-type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename="log_de_usuarios.csv"');
		
		echo 'nombre,descripcion,fecha';
		echo "\r\n";

		$q = "select id,(select username from gs_users where id=id_user) as nombre, event as evento, event_date as fecha from gs_binnacle
		 where id_user in (select id from gs_users where manager_id='" . $_SESSION['user_id'] . "')";

		$q .=  " AND event_date > date_sub(now(), interval 3 MONTH)";
		...
		while ($row = mysqli_fetch_array($r)) {
			...
			echo '"' . $row['nombre'] . '","' . $row['evento'] . '","' . $row['fecha'] . '"';
			echo "\r\n";
		}
	}	
	die;
