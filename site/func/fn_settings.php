<?
	...
	{
		// dashboard
		if (($_SESSION['dashboard'] == '') || (json_decode($_SESSION['dashboard'],true) == null))
		{
			$dashboard = array('open_after_login' => false);
		}
		else
		{
			$dashboard = json_decode($_SESSION['dashboard'], true);
		}
		...
		// ohc
		$default = array(	'stop_engine' => false,
							'stop_engine_color' => '#A5A5A5',
							'no_connection' => false,
		...
		else
		{
			...
			if (!isset($ohc["stop_engine"])) { $ohc["stop_engine"] = $default["stop_engine"]; }
			if (!isset($ohc["stop_engine_color"])) { $ohc["stop_engine_color"] = $default["stop_engine_color"]; }
			...
			if (!isset($ohc["time_idle"])) { $ohc["time_idle"] = $default["time_idle"]; }
		}
		...
		if (is_array($privileges_json)) {
			$perm_array = array_filter(
				$privileges_json,
				fn($v, $k) => strpos($k, 'perm_') === 0,
		...
		$usage_webhook_daily_cnt = $row['usage_webhook_daily_cnt'];
		...
		$result = array('username' => $_SESSION["username"],
						...
						'privileges_dashboard' => $_SESSION["privileges_dashboard"],
						'privileges_history' => $_SESSION["privileges_history"],
						'privileges_reports' => $_SESSION["privileges_reports"],
						'privileges_tachograph' => $_SESSION["privileges_tachograph"],
						...
						'privileges_expenses' => $_SESSION["privileges_expenses"],
						...
						'privileges_events' => $_SESSION["privileges_events"],
						...
						'usage_webhook_daily' => $_SESSION["usage_webhook_daily"],
						...
						'usage_webhook_daily_cnt' => $usage_webhook_daily_cnt,
						'usage_api_daily_cnt' => $usage_api_daily_cnt
						);
		
						$result = array_merge($result, $perm_array);
		echo json_encode($result);
		die;
	}
