	...
	if ($count >= 10) {
		...
		if ($row = mysqli_fetch_array($r)) {
			if ($row['active'] == 'true') {
				...
				if ($_SESSION["privileges"] == 'user'  && $privileges['show_update_dialog'] === true && ($dia == $dia_aviso1 || $dia == $dia_aviso2)) {
					echo 'SOME_OBJECTS_EXCEEDED';
					addRowBinnacle($_SESSION["user_id"], 'Alerta, recordatorio de pago', 'Usuario enterado de Recordatorio de pago');
				} elseif ($_SESSION["privileges"] == 'admin' || $_SESSION["privileges"] == 'super_admin') {
					echo 'LOGIN_CPANEL';
				} else {
					echo 'LOGIN_TRACKING';
				}
...
if (@$_POST['cmd'] == 'logout') {
	...
	if (isset($_SESSION["user_id"])) {
		addRowBinnacle($_SESSION["user_id"], 'cierre de sesión', 'el usuario salió plataforma');
