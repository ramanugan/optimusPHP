<? 
	...
	if(@$_POST['cmd'] == 'delete_object_sensor')
	{
		...
		$r = mysqli_query($ms, $q);
		addRowBinnacle($_SESSION["user_id"], 'Se elimina sensor:' . $sensor_id . " gps: " . $imei, $q);
	...
	if(@$_POST['cmd'] == 'delete_selected_object_sensors')
	{
		...
		for ($i = 0; $i < count($items); ++$i)
		{
			...
			$r = mysqli_query($ms, $q);
			addRowBinnacle($_SESSION["user_id"], 'Se elimina sensor:' . $item . " gps: " . $imei, $q);
		}
	...
	if(@$_POST['cmd'] == 'save_object_sensor')
	{
		...
		addRowBinnacle($_SESSION["user_id"], 'Creación de sensor:' . $param . " gps: " . $name, $q);
